...
foreach d in `dlist' {	
	...
	if _rc==0 {
		if  e(converged)==1 {
			...
			if `levels'==1 {
				survci, surv  range(0 `exrange') outfile(`d', replace)
...
if "`if'"!="" & `levels'== 2 {
	sts graph `if' & real==1, ci by(`varlist') addplot(`myadd', sort ///
	...
	sts graph `if' & real==1, ci addplot(`myadd', sort ///
	legend(order(`"`mylegend'"'))) ytitle("`ytitle'") xtitle("`xtitle'") ///
	xla(0 (`intrange') `exrange') title("`mtitle'")
}
else if "`if'"=="" & `levels' == 1 {
